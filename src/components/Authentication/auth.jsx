...
const Auth = ({authTheme, setRegistered, registered, setFLetter}) => {
    ...
    async function handleAuth() {
        if (newUser) {
            // Sign Up flow
            try {
                const response = await fetch('https://ai-text-summarizer-9lix.onrender.com/signUp', {
                    method: 'POST',
                    credentials: 'include',
            ...
            // Login flow
            try {
                const response = await fetch('https://ai-text-summarizer-9lix.onrender.com/login', {
                    method: 'POST',
                    credentials: 'include',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
    ...
    function handleGoogleAuth() {
        window.location.href = 'https://ai-text-summarizer-9lix.onrender.com/auth/google';
    }
    */
